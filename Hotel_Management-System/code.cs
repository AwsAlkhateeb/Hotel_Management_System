...
{ 
    ...
    class HotelSystem
    {
        ...
        public static void DisplayAllPayments()
        {
            if (File.Exists("Payments.txt"))
            {
                ...
                {
                    Console.WriteLine("Bill Number: " + payment.BillNumber + ", Guest National ID: " + payment.GuestNationalID + ", Source: " + payment.Source + ", Amount: " + payment.Amount + ", Status: " + payment.Status);
                }
            }
            else
            {
    ...
    abstract class User
    {
        ...
        public void DisplayOptionsMenuForGuest()
        {
            ...
            while (x)
            {
                ...
                {
                    case "1":
                        m.ViewAllRooms(HotelSystem.ReturnRoomList().FindAll(r => r.IsAvailable));
                    ...
                    case "3":
                        ...
                        {
                            Service service = new Service(serviceID, reservationIDService, serviceDescription, count);
                    ...
                    case "5":
                        ...
                        break;
                    case "6":
                        Console.Write("Enter service ID to pay for: ");
    ...
    class Guest : User
    {
        ...
        public void ReserveRoom(Room room, string checkInDate, string checkOutDate)
        {
            ...
            { 
                ...
                Console.Write("Create bill number: ");
                ...
                Payment payment = new Payment(billNumber, NationalID, rID, "room reservations", r1.TotalAmount, "not paid");
                HotelSystem.StorePayment(payment);
                Reservations.Add(r1);
        ...
        public void RequestService(Service service, Reservation reservation, List<Service> servicesList)
        {
            ...
            {
                service.GuestNationalID = reservation.GuestNationalID;
                HotelSystem.StoreService(service);
                servicesList.Add(service);
                Console.Write("Create bill number: ");
                string billNumber = Console.ReadLine();
                Payment payment = new Payment(billNumber, reservation.GuestNationalID, service.ID, service.Description.ToLower(), service.Cost, "not paid");
                HotelSystem.StorePayment(payment);
        ...
        public void PayForReservation(Reservation reservation)
        {
            ...
            if (BankBalance >= amount)
            {
                ...
                List<Payment> plist = HotelSystem.ReturnPaymentList();
                for (int i = 0; i < plist.Count; i++)
                {
                    if (plist[i].RelatedID == reservation.ID)
                    {
                        plist[i].Status = status;
                        break;
                    }
                }

                FileStream fileStream = new FileStream("Payments.txt", FileMode.Create);
                BinaryFormatter formatter = new BinaryFormatter();
                foreach (Payment updatedPayment in plist)
                {
                    formatter.Serialize(fileStream, updatedPayment);
                }
                fileStream.Close();
        ...
        public void PayForService(Service service)
        {
            ...
            if (BankBalance >= amount)
            {
                ...
                for (int i = 0; i < plist.Count; i++)
                {
                    if (plist[i].RelatedID == service.ID)
                    {
                        plist[i].Status = status;
                        break;
                    }
        ...
        public void GenerateProfitReport(List<Payment> payments, List<Reservation> reservations, List<Service> services)
        {
            ...
            foreach (Payment payment in payments)
            {
                if (payment.Status == "paid")
                {
                    ...
                    if (payment.Source == "room reservations")
                    {
                        // مرتبط بحجز غرفة
                        totalRoomIncome += payment.Amount;
                    }
                    else if (payment.Source == "car rental")
                    {
                        // مرتبط بتأجير سيارة
                        totalCarRentalIncome += payment.Amount;
                    }
                    else if (payment.Source == "kids zone")
                    {
    ...
    class Service
    {
        ...
        public override string ToString()
        {
            return "[Service]\nID: " + ID + ", Guest National ID: " + GuestNationalID + ", Reservation ID: " + ReservationID + ", Description: " + Description + ", Cost: " + Cost + ", Notes: " + Notes;
        }
    }
    ...
    class Payment
    {
        ...
        public Payment(string billNumber, string guestNationalID, string relatedID, string source, double amount,string status)
        {
        ...
        public override string ToString()
        {
            return "[Payment]\nBill Number: " + BillNumber + ", Guest National ID: " + GuestNationalID + ", Related ID: " + RelatedID + ", Source: " + Source + ", Amount: " + Amount + ", Status: " + Status;
        }
    }
    class Program
    {
        ...
        {
            if (!File.Exists("Guests.txt") || new FileInfo("Guests.txt").Length == 0)
            {
            ...
            if (!File.Exists("Rooms.txt") || new FileInfo("Rooms.txt").Length == 0)
            {
